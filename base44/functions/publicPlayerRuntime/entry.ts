...
function pickPlayerPublic(player: Record<string, any>) {
  return { id: player.id, name: player.name || "StudioSoundSet Player", email: player.email || "", providerId: player.providerId || "", zoneId: player.zoneId || "", isActive: player.isActive !== false, isOnline: !!player.isOnline, sdkReady: !!player.sdkReady, sdkConnected: !!player.sdkConnected, spotifyDeviceId: player.spotifyDeviceId || "", lastError: player.lastError || "", lastSeen: player.lastSeen || "", lastHeartbeatAt: player.lastHeartbeatAt || "" };
}
...
function sanitizeHeartbeat(payload: Record<string, any> = {}) {
  return { lastSeen: nowIso(), lastHeartbeatAt: nowIso(), isOnline: true, sdkReady: !!payload.sdkReady, sdkConnected: !!payload.sdkConnected, spotifyDeviceId: String(payload.spotifyDeviceId || ""), currentTrackName: String(payload.currentTrackName || ""), currentTrackArtist: String(payload.currentTrackArtist || ""), currentTrackAlbum: String(payload.currentTrackAlbum || ""), currentTrackCoverUrl: String(payload.currentTrackCoverUrl || ""), currentTrackUri: String(payload.currentTrackUri || ""), currentPlaylistUri: String(payload.currentPlaylistUri || ""), isPlaying: !!payload.isPlaying, progressMs: Number.isFinite(Number(payload.progressMs)) ? Number(payload.progressMs) : 0, durationMs: Number.isFinite(Number(payload.durationMs)) ? Number(payload.durationMs) : 0, volume: Number.isFinite(Number(payload.volume)) ? Number(payload.volume) : undefined, lastError: String(payload.lastError || ""), playbackStateAvailable: payload.playbackStateAvailable !== false, lastStatusUpdate: nowIso() };
}
...
function validatePlayerSession(player: Record<string, any>, sessionToken: string) {
  ...
  if (!validTokens.includes(String(sessionToken))) throw Object.assign(new Error("Player Session ist ungültig."), { status: 403, code: "SESSION_TOKEN_INVALID" });
}
...
async function getProviderAccessToken(base44: any, provider: Record<string, any>) {
  ...
  const body = new URLSearchParams({ grant_type: "refresh_token", refresh_token: provider.refreshToken, client_id: provider.clientId });
...
async function handleBootstrap(base44: any, player: Record<string, any>) {
  ...
  await base44.asServiceRole.entities.Player.update(player.id, { lastSeen: nowIso(), lastHeartbeatAt: nowIso(), isOnline: true, lastError: player.providerId ? "" : "Player has no providerId assigned." });
  return { success: true, player: pickPlayerPublic(player), provider: pickProviderPublic(provider), zone: zone ? { id: zone.id, name: zone.name || "", providerId: zone.providerId || "" } : null };
}
...
async function handlePollCommands(base44: any, player: Record<string, any>) {
  ...
  await base44.asServiceRole.entities.PlayerCommand.update(command.id, { status: COMMAND_STATUS.PICKED_UP, pickedUpAt: nowIso(), humanMessage: "Player picked up command and is executing it." });
  ...
  return { success: true, command: { id: command.id, playerId: command.playerId, providerId: command.providerId || player.providerId || "", zoneId: command.zoneId || player.zoneId || "", type: command.type || command.command, command: command.command || command.type, payload: command.payload || {}, createdAt: command.createdAt || command.created_date || "" } };
}
async function handleCommandResult(base44: any, player: Record<string, any>, payload: Record<string, any>) {
  ...
  const patch = { status, completedAt: nowIso(), result: payload.result || {}, errorCode: payload.errorCode || "", humanMessage: payload.humanMessage || (status === COMMAND_STATUS.SUCCESS ? "Command confirmed by Player." : "Command failed on Player."), technicalMessage: payload.technicalMessage || "", suggestedFix: payload.suggestedFix || "" };
  await base44.asServiceRole.entities.PlayerCommand.update(command.id, patch);
  await base44.asServiceRole.entities.Player.update(player.id, { lastSeen: nowIso(), lastHeartbeatAt: nowIso(), isOnline: true, lastCommand: command.type || command.command || "", lastCommandStatus: status, lastError: status === COMMAND_STATUS.SUCCESS ? "" : patch.humanMessage, lastCommandCompletedAt: nowIso() });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!["bootstrap", "heartbeat", "pollCommands", "commandResult", "getAccessToken", "playPlaylist"].includes(action)) return json({ success: false, errorCode: "ACTION_INVALID", error: "Invalid publicPlayerRuntime action." }, { status: 400 });
