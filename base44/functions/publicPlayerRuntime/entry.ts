...
function pickPlayerPublic(player: Record<string, any>) {
  return { id: player.id, name: player.name || "StudioSoundSet Player", email: player.email || "", providerId: player.providerId || "", zoneId: player.zoneId || "", isActive: player.isActive !== false, isOnline: !!player.isOnline, sdkReady: !!player.sdkReady, sdkConnected: !!player.sdkConnected, spotifyDeviceId: player.spotifyDeviceId || "", currentTrackName: player.currentTrackName || "", currentTrackArtist: player.currentTrackArtist || "", currentTrackAlbum: player.currentTrackAlbum || "", currentTrackCoverUrl: player.currentTrackCoverUrl || "", isPlaying: !!player.isPlaying, volume: Number.isFinite(Number(player.volume)) ? Number(player.volume) : 50, lastError: player.lastError || "", lastSeen: player.lastSeen || "", lastHeartbeatAt: player.lastHeartbeatAt || "" };
}
...
async function validatePlayerSession(base44: any, player: Record<string, any>, sessionToken: string) {
  ...
  const storedRuntimeSession = await getRuntimeSession(base44, player.id);
  const validTokens = [player.sessionToken, player.setupToken, storedRuntimeSession?.sessionToken, storedRuntimeSession?.setupToken].filter(Boolean).map(String);
...
async function handleListPlaylists(base44: any, player: Record<string, any>) {
  ...
  const playlists = merged.filter((playlist: any) => { if (!playlist?.id || seen.has(playlist.id)) return false; seen.add(playlist.id); return !playlist.playerId || playlist.playerId === player.id || playlist.providerId === player.providerId || playlist.spotifyAccountId === player.providerId; }).map(pickPlaylistPublic).sort((a: any, b: any) => String(a.name).localeCompare(String(b.name)));
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await validatePlayerSession(base44, player, sessionToken);
