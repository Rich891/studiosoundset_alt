...
async function getAccessToken(base44: any, provider: Record<string, any>) {
  ...
  const response = await fetch(TOKEN_URL, {
    ...
    body: new URLSearchParams({ grant_type: "refresh_token", refresh_token: provider.refreshToken, client_id: provider.clientId }),
  });
...
async function spotifyApi(path: string, { method = "GET", accessToken, body }: Record<string, any>) {
  ...
  if (!response.ok) {
    ...
    throw Object.assign(new Error(message), { status: response.status, code: "SPOTIFY_API_FAILED" });
  }
  return data;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({ success: false, errorCode: error?.code || "SPOTIFY_ACCOUNT_CONTROL_ERROR", error: error?.message || "spotifyAccountControl failed." }, { status });
  }
});
