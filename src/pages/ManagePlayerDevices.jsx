...
import { getPlayerProviderId, getProviderDisplayName, providerStatus } from '@/lib/playerAssignments';
...
export default function ManagePlayerDevices() {
  ...
  const { data: players = [], error: playerError } = useQuery({ queryKey: ['players'], queryFn: () => base44.entities.Player.list('-updated_date'), refetchInterval: 5000 });
  const { data: providers = [] } = useQuery({ queryKey: ['providers'], queryFn: () => base44.entities.Provider.list('-created_date'), refetchInterval: 15000 });
  const { data: zones = [] } = useQuery({ queryKey: ['zones'], queryFn: () => base44.entities.Zone.list(), refetchInterval: 15000 });
  ...
  const persistPlayerAssignment = async (player, providerId, zoneId, options = {}) => {
    ...
    if (!providerId) throw new Error('API-Verbindung fehlt. Waehle einen verbundenen Provider.');
    const provider = providersById[providerId];

    const response = await invoke('playerAdminControl', {
      action: 'ensureAssignment',
      playerId: player.id,
      providerId,
      zoneId: zoneId || '',
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (player) => {
      refresh();
      toast.success('Player erstellt, API-Zuweisung und Runtime Session im Backend gespeichert.');
      openSetup(player, { skipEnsure: true });
      setShowDialog(false);
      setFormData(EMPTY_FORM);
    },
    onError: (err) => toast.error(err.humanMessage || err.message),
  });

  const updateAssignmentMutation = useMutation({
    mutationFn: async ({ player, providerId, zoneId }) => persistPlayerAssignment(player, providerId, zoneId === 'none' ? '' : zoneId, { forceNewSession: true }),
    onSuccess: (player) => {
      refresh();
      toast.success('Player-Zuweisung und Runtime Session gespeichert. Bitte neuen Player-Link oeffnen.');
      setEditAssignment(EMPTY_EDIT);
      openSetup(player, { skipEnsure: true });
    },
    onError: (err) => toast.error(`Zuweisung konnte nicht gespeichert werden: ${err.humanMessage || err.message}`),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.Player.delete(id),
    onSuccess: () => { refresh(); toast.success('Player geloescht.'); },
    onError: (err) => toast.error(`Player konnte nicht geloescht werden: ${err.message}`),
  });
  ...
  const openZoneDialog = (target) => { setZoneTarget(target); setZoneForm(EMPTY_ZONE); setShowZoneDialog(true); };
  const copyToClipboard = (text) => { navigator.clipboard.writeText(text || ''); toast.success('Kopiert.'); };
  const generatePassword = () => { const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; let pwd = ''; for (let i = 0; i < 10; i += 1) pwd += chars.charAt(Math.floor(Math.random() * chars.length)); setFormData({ ...formData, passwordHash: pwd }); };

  const openSetup = async (player, options = {}) => {
    let verified = player;
    const providerId = getPlayerProviderId(verified);
    if (!providerId) { toast.error('Dieser Player hat keine API-Verbindung. Zuweisung bearbeiten.'); return; }
    try {
      if (!options.skipEnsure) verified = await persistPlayerAssignment(verified, providerId, verified.zoneId || '');
      refresh();
    } catch (e) {
      toast.error(`Runtime Session konnte nicht erzeugt werden: ${e.humanMessage || e.message}`);
      return;
    }
    const provider = providersById[providerId];
    setSetupData({ playerId: verified.id, providerId, providerClientId: provider?.clientId || verified.spotifyClientId || '', zoneId: verified.zoneId || '', sessionToken: verified.sessionToken || verified.setupToken || '', deviceName: verified.name });
  ...
  const getZoneName = (player) => zonesById[player.zoneId]?.name || player.zoneName || '-';
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid gap-4">
        {players.length === 0 ? <motion.div className="bento-panel border-dashed border-primary/20 p-12 text-center"><Music2 className="w-12 h-12 text-muted-foreground/20 mx-auto mb-4" /><h3 className="text-lg font-bold mb-2">Keine Player vorhanden</h3><p className="text-muted-foreground mb-6">Erstelle den ersten Player und oeffne den Setup-Link auf dem Player-Geraet.</p><Button onClick={() => setShowDialog(true)} className="bg-primary">Neuer Player</Button></motion.div> : players.map((player, idx) => {
          const providerId = getPlayerProviderId(player); const provider = providersById[providerId]; const online = isPlayerOnline(player); const isConnected = providerStatus(provider) === 'connected'; const hasRuntimeToken = !!(player.sessionToken || player.setupToken);
          return <motion.div key={player.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }}><div className="bento-panel p-5 space-y-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-xl font-black">{player.name}</h3><div className="flex items-center gap-2 mt-1 flex-wrap">{online ? <Wifi className="w-3.5 h-3.5 text-green-400" /> : <WifiOff className="w-3.5 h-3.5 text-yellow-400" />}<span className={`text-xs font-semibold ${online ? 'text-green-400' : 'text-yellow-400'}`}>{online ? 'Online' : 'Offline'}</span>{player.sdkReady && <span className="text-xs text-cyan-400">SDK Ready</span>}{providerId && <span className="text-xs text-green-400 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> API am Player</span>}<span className={`text-xs ${hasRuntimeToken ? 'text-green-400' : 'text-red-300'}`}>{hasRuntimeToken ? 'Runtime Session OK' : 'Runtime Session fehlt'}</span></div></div><div className="flex items-center gap-1"><Button variant="ghost" size="sm" onClick={() => openSetup(player)} className="h-8 w-8 p-0" title="Player oeffnen"><QrCode className="w-4 h-4" /></Button><Button variant="ghost" size="sm" onClick={() => deleteMutation.mutate(player.id)} className="text-red-400 hover:text-red-300 h-8 w-8 p-0" disabled={deleteMutation.isPending}><Trash2 className="w-4 h-4" /></Button></div></div><div className="grid md:grid-cols-4 gap-3 text-sm"><div className={`rounded-lg p-3 border ${providerId ? 'bg-muted/20 border-border/30' : 'bg-red-500/5 border-red-500/20'}`}><p className="text-xs text-muted-foreground font-semibold mb-1">API-Verbindung</p><p className="font-bold text-sm">{provider ? getProviderDisplayName(provider) : providerId ? 'Provider nicht gefunden' : 'Nicht zugewiesen'}</p><p className={`text-xs mt-1 ${isConnected ? 'text-green-400' : providerId ? 'text-yellow-400' : 'text-red-300'}`}>{provider ? providerStatus(provider) : providerId ? 'am Player gespeichert' : 'Keine API-Verbindung'}</p></div><div className="bg-muted/20 rounded-lg p-3 border border-border/30"><p className="text-xs text-muted-foreground font-semibold mb-1">Zone</p><p className="font-bold text-sm">{getZoneName(player)}</p><p className="text-xs text-muted-foreground mt-1">optional</p></div><div className="bg-muted/20 rounded-lg p-3 border border-border/30"><p className="text-xs text-muted-foreground font-semibold mb-1">Device ID</p><p className="font-mono text-xs break-all">{player.spotifyDeviceId || '-'}</p></div><div className="bg-muted/20 rounded-lg p-3 border border-border/30"><p className="text-xs text-muted-foreground font-semibold mb-1">Last Command</p><p className="text-xs break-all">{player.lastCommand || '-'} {player.lastCommandStatus ? `- ${player.lastCommandStatus}` : ''}</p></div></div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" onClick={() => openEditAssignment(player)} className="gap-2"><Wrench className="w-4 h-4" />Zuweisung bearbeiten</Button><Button variant="outline" size="sm" onClick={() => openSetup(player)} className="gap-2"><QrCode className="w-4 h-4" />Player-Link</Button></div><div className="bg-background/50 rounded-lg p-3 space-y-2 border border-border/30"><p className="text-xs text-muted-foreground font-semibold">Player Login</p><div className="flex items-center gap-2 font-mono text-xs"><code className="flex-1 bg-background px-2 py-1.5 rounded border border-border/30 truncate">{player.email}</code><Button variant="ghost" size="sm" onClick={() => copyToClipboard(player.email)} className="h-7 w-7 p-0"><Copy className="w-3 h-3" /></Button></div><div className="flex items-center gap-2 font-mono text-xs"><code className="flex-1 bg-background px-2 py-1.5 rounded border border-border/30 truncate">{showPassword[player.id] ? player.passwordHash : '********'}</code><Button variant="ghost" size="sm" onClick={() => setShowPassword(prev => ({ ...prev, [player.id]: !prev[player.id] }))} className="h-7 w-7 p-0">{showPassword[player.id] ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}</Button><Button variant="ghost" size="sm" onClick={() => copyToClipboard(player.passwordHash)} className="h-7 w-7 p-0"><Copy className="w-3 h-3" /></Button></div></div>{player.lastError && <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3 text-xs text-red-300">{player.lastError}</div>}</div></motion.div>;
        })}
      </div>
