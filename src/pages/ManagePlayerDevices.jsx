...
export default function ManagePlayerDevices() {
  ...
  const persistPlayerAssignment = async (player, providerId, zoneId, options = {}) => {
    ...
    const verified = { ...player, ...response.data.player };
    ...
    await savePlayerConfig(verified, {
      providerId,
      providerName: provider?.name || provider?.displayName || '',
      spotifyClientId: provider?.clientId || verified.spotifyClientId || '',
      zoneId: zoneId || '',
      zoneName: zone?.name || '',
  ...
  const updateAssignmentMutation = useMutation({
    mutationFn: async ({ player, providerId, zoneId }) => persistPlayerAssignment(player, providerId, zoneId === 'none' ? '' : zoneId),
    onSuccess: (player) => {
      refresh();
  ...
  const generatePassword = () => { const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; let pwd = ''; for (let i = 0; i < 10; i++) pwd += chars.charAt(Math.floor(Math.random() * chars.length)); setFormData({ ...formData, passwordHash: pwd }); };
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between gap-4 flex-wrap"><div><h1 className="text-2xl font-black">Players</h1><p className="text-sm text-muted-foreground mt-1">Provider/API gehört direkt an den Player. Zonen sind nur optionale Räume und Default-Werte.</p></div><Button onClick={() => setShowDialog(true)} className="gap-2 bg-primary hover:bg-primary/90"><Plus className="w-4 h-4" />Neuer Player</Button></div>
      <div className="bento-panel p-5 border-cyan-500/20 bg-cyan-500/5"><div className="flex gap-3"><PlugZap className="w-5 h-5 text-cyan-300 mt-0.5 flex-shrink-0" /><div className="space-y-1 text-sm"><p className="font-black text-cyan-200">Optimierter Flow</p><p className="text-muted-foreground">Provider verbinden → Player erstellen/Provider zuweisen → Zone optional hier erstellen/setzen → Player-Link öffnen. Runtime Sessions werden über playerAdminControl im Backend gespeichert.</p></div></div></div>
