...
import {
  ...
  Trash2, Link2, Link2Off, Pencil, Copy
} from 'lucide-react';
...
export default function SpotifyAccounts() {
  ...
  const { data: providers = [], isLoading } = useQuery({ queryKey: ['providers'], queryFn: () => base44.entities.Provider.list('-created_date') });

  const createMutation = useMutation({
    mutationFn: (data) => base44.functions.invoke('providerControl', { action: 'create', data }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['providers'] }); closeDialog(); toast.success('Provider erstellt.'); },
  });
  ...
  const openEdit = (provider) => {
    ...
    setForm({ displayName: provider.name || '', clientId: provider.clientId || '', clientSecret: '' });
    setShowDialog(true);
  };
  const closeDialog = () => { setShowDialog(false); setEditAccount(null); setForm(EMPTY_FORM); };

  const handleSave = () => {
    const payload = { name: form.displayName.trim(), clientId: form.clientId.trim() };
    ...
    try {
      const res = await base44.functions.invoke('spotifyAuth', {
        ...
        scopes: [
          ...
          'user-read-currently-playing', 'playlist-read-private', 'playlist-read-collaborative', 'streaming'
        ].join(' ')
      });
      if (res.data?.url) window.location.href = res.data.url;
      else toast.error(res.data?.error || 'Auth URL konnte nicht geladen werden.');
    } catch (e) {
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 max-w-5xl mx-auto">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-black flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-green-500/10 border border-green-500/20 flex items-center justify-center"><Music2 className="w-5 h-5 text-green-400" /></div>Spotify Provider</h1>
          <p className="text-sm text-muted-foreground mt-1 ml-14">Verbinde deine Spotify Premium Accounts.</p>
        </div>
      ...
      <div className="bento-panel p-4 flex items-start gap-3 border-blue-500/20 bg-blue-500/5">
        ...
        <div className="text-sm space-y-2 flex-1">
          ...
          <p className="text-muted-foreground">Diese URI muss exakt im Spotify Developer Dashboard stehen. Wenn sich die Base44-Domain ändert, diese URI erneut eintragen.</p>
          <div className="flex gap-2 items-center">
            <code className="block bg-muted/40 rounded-lg px-3 py-2 text-xs text-blue-200 font-mono flex-1 break-all">{redirectUri}</code>
            <Button variant="outline" size="sm" onClick={copyRedirect} className="gap-2"><Copy className="w-3.5 h-3.5" /> Kopieren</Button>
          </div>
          {!isHttps && <p className="text-xs text-red-300">Achtung: Spotify OAuth benötigt HTTPS, außer bei localhost/127.0.0.1 Development.</p>}
        </div>
      </div>

      {providers.length === 0 ? (
        <div className="bento-panel border-dashed border-primary/20 p-16 text-center"><Music2 className="w-16 h-16 text-muted-foreground/20 mx-auto mb-4" /><h3 className="text-xl font-bold mb-2">Noch keine Provider</h3><p className="text-muted-foreground text-sm mb-6">Verbinde deine Spotify Premium Accounts.</p><Button className="bg-primary hover:bg-primary/90" onClick={openCreate}><Plus className="w-4 h-4 mr-2" /> Ersten Provider erstellen</Button></div>
      ) : (
                  ...
                  <div className={`bento-panel border ${cfg.bg} overflow-hidden`}>
                    <div className="p-5 flex items-center gap-4 flex-wrap">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 flex-wrap mt-0.5"><span className={`text-xs font-semibold ${cfg.color}`}>{cfg.label}</span>{provider.spotifyUserEmail && <span className="text-xs text-muted-foreground">{provider.spotifyUserEmail}</span>}{provider.clientId && <span className="text-xs text-muted-foreground font-mono">ID: {provider.clientId.substring(0, 8)}...</span>}</div>
                        {provider.lastError && <p className="text-xs text-red-400 mt-1">⚠ {provider.lastError}</p>}
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <Button size="sm" className="gap-1.5 bg-green-600 hover:bg-green-700 text-white font-semibold" onClick={() => handleConnect(provider)}><Link2 className="w-3.5 h-3.5" /> {provider.status === 'connected' ? 'Erneut verbinden' : 'Mit Spotify verbinden'}</Button>
                        <Button variant="ghost" size="sm" className="gap-1" onClick={() => openEdit(provider)}><Pencil className="w-4 h-4" /></Button>
                        <Button variant="ghost" size="sm" className="text-destructive/70 hover:text-destructive" onClick={() => deleteMutation.mutate(provider.id)}><Trash2 className="w-4 h-4" /></Button>
                      </div>
                    </div>
