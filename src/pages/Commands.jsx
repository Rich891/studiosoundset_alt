...
export default function Commands() {
  ...
  const { data: commands = [], isLoading, error } = useQuery({ queryKey: ['playerCommands'], queryFn: () => base44.entities.PlayerCommand.list('-createdAt'), refetchInterval: 3000 });

  useEffect(() => { players.forEach((player) => markStalePendingCommands(player.id)); }, [players]);
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      {error && <div className="bento-panel border-red-500/20 bg-red-500/5 p-4 text-sm text-red-300"><p className="font-bold">PlayerCommand Entity fehlt oder ist nicht lesbar.</p><p className="mt-1">{error.message}</p><p className="mt-2 text-red-200/80">Lege in Base44 die Entity `PlayerCommand` mit den Feldern aus `docs/studiosoundset-base44-port.md` an. Ohne diese Entity gibt es keinen sicheren Command-Handshake.</p></div>}
