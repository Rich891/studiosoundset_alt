...
import { COMMAND_STATUS, listPlayerCommands, markStalePendingCommands } from '@/lib/studioSoundSetRuntime';
...
export default function Commands() {
  ...
  const { data: commands = [], isLoading, error } = useQuery({ queryKey: ['playerCommands'], queryFn: () => listPlayerCommands(), refetchInterval: 3000 });
  ...
  return (
    <div className="p-4 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      {error && <div className="bento-panel border-red-500/20 bg-red-500/5 p-4 text-sm text-red-300"><p className="font-bold">Command Backend nicht erreichbar.</p><p className="mt-1">{error.message}</p><p className="mt-2 text-red-200/80">Commands laufen über die Backend-Funktion `playerCommandControl`, nicht über eine öffentliche PlayerCommand Entity.</p></div>}
