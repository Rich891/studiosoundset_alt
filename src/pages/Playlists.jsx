...
async function importTracks({ provider, playlist, spotifyPlaylistId }) {
  ...
  if (!res.data?.success) {
    await base44.entities.Playlist.update(playlist.id, {
      ...
      syncStatus: 'error',
      lastError: res.data?.error || 'TRACK_IMPORT_FAILED',
      lastTrackSyncAt: new Date().toISOString(),
    });
    throw new Error(res.data?.error || 'Track-Import fehlgeschlagen.');
  }

  const imported = Number(res.data.imported || res.data.importedTracks || 0);
  const total = Number(playlist.totalTracks || 0);
...
function SpotifyPlaylistPicker({ provider, player, importedPlaylists, onImported, onClose }) {
  ...
  const loadPlaylists = async () => {
    ...
    try {
      ...
      if (!res.data?.success) throw new Error(res.data?.error || 'Fehler beim Laden der Playlists.');
  ...
  const handleImport = async (pl) => {
    ...
    try {
      ...
      toast.error(e.message.includes('403') || e.message.includes('Forbidden') ? 'Kein Zugriff auf diese Playlist. Spotify User/Scopes prüfen und Provider neu verbinden.' : e.message);
      onImported();
    } finally {
      setImporting(null);
    }
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="bg-card border-border max-w-4xl flex flex-col" style={{ maxHeight: '85vh' }}>
        ...
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">Importiert für Player: <strong>{player?.name || 'kein Player ausgewählt'}</strong></p>
          <Button onClick={loadPlaylists} disabled={loading} className="gap-2"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /> Load Spotify Playlists</Button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 grid md:grid-cols-2 gap-3 pr-1">
          {spotifyPlaylists.map(pl => {
            ...
            return (
              <div key={pl.id} className="rounded-xl border border-border/40 bg-background/40 p-3 space-y-3">
                <div className="flex gap-3">
                  {pl.images?.[0]?.url ? <img src={pl.images[0].url} alt={pl.name} className="w-16 h-16 rounded-lg object-cover" /> : <div className="w-16 h-16 bg-muted/30 rounded-lg flex items-center justify-center"><Music2 className="w-6 h-6 text-muted-foreground" /></div>}
                  <div className="min-w-0 flex-1">
                    <p className="font-bold truncate">{pl.name}</p>
                    <p className="text-xs text-muted-foreground truncate">{pl.owner?.display_name || pl.owner?.id} · {pl.tracks?.total ?? 0} tracks</p>
                    <p className="text-xs text-muted-foreground truncate">{pl.uri}</p>
                    <p className={already ? 'text-xs text-green-400' : 'text-xs text-muted-foreground'}>{already ? 'Already imported' : 'Not imported'}</p>
                  </div>
                </div>
                <Button className="w-full gap-2" variant={already ? 'outline' : 'default'} onClick={() => handleImport(pl)} disabled={importing === pl.id || !player?.id}>
                  {importing === pl.id ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />} Import
                </Button>
              </div>
            );
          })}
...
export default function Playlists() {
  ...
  const filtered = useMemo(() => playlists.filter(pl => {
    if (search && !pl.name?.toLowerCase().includes(search.toLowerCase())) return false;
    if (filterProviderId !== 'all' && pl.providerId !== filterProviderId && pl.spotifyAccountId !== filterProviderId) return false;
    if (filterStatus !== 'all' && (pl.trackSyncStatus || pl.syncStatus) !== filterStatus) return false;
    return true;
  ...
  const importByInput = async () => {
    ...
    try {
      ...
      toast.error(e.message.includes('403') || e.message.includes('Forbidden') ? 'PLAYLIST_NOT_ACCESSIBLE: Diese Playlist kann mit diesem Spotify Account nicht geladen werden.' : e.message);
      refresh();
    }
  };
  ...
  const handlePlay = async (pl) => {
    const targetPlayer = players.find(p => p.id === (pl.playerId || selectedPlayer?.id)) || selectedPlayer;
    if (!targetPlayer) return toast.error('Kein Player ausgewählt.');
    await createPlayerCommand(targetPlayer, COMMAND.PLAY_PLAYLIST, { playlistId: pl.id, contextUri: pl.providerPlaylistUri });
    toast.success('PLAY_PLAYLIST gesendet. Erfolg erscheint erst nach Player-Bestätigung.');
  };
        ...
        <div className="grid lg:grid-cols-[1fr_auto] gap-3 items-end">
          <div>
            ...
            <Select value={selectedPlayer?.id || ''} onValueChange={setSelectedPlayerId}>
              <SelectTrigger className="h-11 bg-background/50"><SelectValue placeholder="Player wählen" /></SelectTrigger>
              <SelectContent>{players.map(p => <SelectItem key={p.id} value={p.id}>{p.name} · {zones.find(z => z.id === p.zoneId)?.name || 'No zone'} · {getProviderLabel(providers.find(pr => pr.id === (p.providerId || zones.find(z => z.id === p.zoneId)?.providerId)))}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <Button className="h-11 gap-2" onClick={() => setShowSpotifyPicker(true)} disabled={!selectedProvider || getProviderStatus(selectedProvider) !== 'connected'}><Download className="w-4 h-4" /> Load Spotify Playlists</Button>
        </div>
        {selectedProvider ? <p className="text-xs text-muted-foreground">Provider: <strong>{getProviderLabel(selectedProvider)}</strong> · Status: <span className={getProviderStatus(selectedProvider) === 'connected' ? 'text-green-400' : 'text-yellow-400'}>{getProviderStatus(selectedProvider)}</span></p> : <p className="text-xs text-yellow-300">Dieser Player hat keinen Spotify Provider. Weise in Zonen/Player verwalten einen Provider zu.</p>}

        <div className="grid lg:grid-cols-[1fr_auto] gap-3 items-end">
          <div>
            <label className="text-sm font-semibold mb-2 block">Spotify Playlist Link or URI</label>
            <Input value={playlistInput} onChange={(e) => setPlaylistInput(e.target.value)} placeholder="https://open.spotify.com/playlist/... oder spotify:playlist:..." className="h-11 bg-background/50" />
          </div>
      ...
      <div className="bento-panel p-5 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            <div className="relative"><Search className="w-4 h-4 absolute left-3 top-3 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search playlists" className="pl-9 h-10 w-52" /></div>
            <Select value={filterProviderId} onValueChange={setFilterProviderId}><SelectTrigger className="w-44 h-10"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Providers</SelectItem>{providers.map(p => <SelectItem key={p.id} value={p.id}>{getProviderLabel(p)}</SelectItem>)}</SelectContent></Select>
            <Select value={filterStatus} onValueChange={setFilterStatus}><SelectTrigger className="w-44 h-10"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All sync status</SelectItem><SelectItem value="success">Success</SelectItem><SelectItem value="partial">Partial</SelectItem><SelectItem value="failed">Failed</SelectItem><SelectItem value="loading">Loading</SelectItem></SelectContent></Select>
          </div>
        </div>
        ...
        {isLoading ? <div className="flex items-center justify-center gap-2 py-10"><RefreshCw className="w-5 h-5 animate-spin text-primary" /></div> : filtered.length === 0 ? <div className="py-12 text-sm text-muted-foreground">No playlists imported yet.</div> : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            <AnimatePresence>
              {filtered.map((pl, i) => {
                const provider = providers.find(a => a.id === (pl.providerId || pl.spotifyAccountId));
                const player = players.find(p => p.id === pl.playerId);
                const zone = zones.find(z => z.id === pl.zoneId || z.id === player?.zoneId);
                const statusKey = pl.trackSyncStatus || pl.syncStatus || 'not_imported';
                const syncCfg = SYNC_CFG[statusKey] || SYNC_CFG.not_imported;
                const missingTracks = (pl.totalTracks || 0) > 0 && (pl.importedTracks || 0) < (pl.totalTracks || 0);
                return (
                  <motion.div key={pl.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                    <div className="bento-panel overflow-hidden group">
                      {pl.coverUrl ? <img src={pl.coverUrl} alt={pl.name} className="w-full aspect-square object-cover" /> : <div className="w-full aspect-square bg-muted/20 flex items-center justify-center"><Music2 className="w-12 h-12 text-muted-foreground/30" /></div>}
                      <div className="p-4 space-y-3">
                        <div><p className="font-bold truncate">{pl.name}</p><p className="text-xs text-muted-foreground truncate">{provider ? getProviderLabel(provider) : 'No provider'} · {zone?.name || 'No zone'}</p></div>
                        <div className="flex items-center justify-between text-xs"><span className={`font-semibold ${syncCfg.color}`}>{syncCfg.label}</span><span className="text-muted-foreground">{pl.importedTracks || 0} / {pl.totalTracks || 0} songs</span></div>
                        ...
                        {pl.lastError && <p className="text-xs text-red-400 break-words">{pl.lastError}</p>}
                        <div className="grid grid-cols-4 gap-2">
                          <Button size="sm" className="gap-1.5 h-8 text-xs col-span-2 bg-green-600 hover:bg-green-700" onClick={() => handlePlay(pl)} disabled={!pl.providerPlaylistUri}><Play className="w-3.5 h-3.5" /> Play</Button>
                          <Button size="sm" variant="outline" className="h-8" onClick={() => setDetailPlaylist(pl)}><Eye className="w-3.5 h-3.5" /></Button>
                          <Button size="sm" variant="outline" className="h-8" onClick={() => handleResync(pl)} disabled={resyncingId === pl.id}><RefreshCw className={`w-3.5 h-3.5 ${resyncingId === pl.id ? 'animate-spin' : ''}`} /></Button>
                        </div>
                        <Button size="sm" variant="ghost" className="w-full h-8 text-destructive/70 hover:text-destructive" onClick={() => deleteMutation.mutate(pl.id)}><X className="w-3.5 h-3.5 mr-1" /> Remove</Button>
                      </div>
                    </div>
                  </motion.div>
                );
      ...
      {detailPlaylist && (
        <Dialog open onOpenChange={() => setDetailPlaylist(null)}>
          <DialogContent className="bg-card border-border max-w-4xl max-h-[85vh] overflow-y-auto">
            <DialogHeader><DialogTitle className="flex items-center gap-3">{detailPlaylist.coverUrl && <img src={detailPlaylist.coverUrl} alt="" className="w-12 h-12 rounded-lg" />}{detailPlaylist.name}</DialogTitle></DialogHeader>
            <div className="grid md:grid-cols-3 gap-3 text-xs text-muted-foreground"><div>Total: {detailPlaylist.totalTracks || 0}</div><div>Imported: {detailPlaylist.importedTracks || 0}</div><div>Status: {detailPlaylist.trackSyncStatus || detailPlaylist.syncStatus}</div></div>
